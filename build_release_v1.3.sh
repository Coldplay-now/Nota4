...
# 复制 Info.plist（包含 CFBundleIconFile）
...
if [ -d "$INITIAL_DOCS_SRC" ]; then
    ...
    if [ -d "$INITIAL_DOCS_DEST" ]; then
        ...
        # 验证关键文件存在（包括文档和图片）
        REQUIRED_FILES=("使用说明.nota" "Markdown完整示例.nota" "运动.nota" "技术.nota")
        REQUIRED_IMAGES=("icon_1024x1024.png" "toolbar.jpeg" "preference.jpeg" "preference2.jpeg")
        MISSING_FILES=()
        MISSING_IMAGES=()
        ...
        if [ ${#MISSING_FILES[@]} -eq 0 ] && [ ${#MISSING_IMAGES[@]} -eq 0 ]; then
            echo "   ✓ InitialDocuments 关键文件验证通过（4个文档 + 4个图片）"
            # 验证文件大小（确保不是空文件）
            for file in "${REQUIRED_FILES[@]}"; do
            ...
            # 验证图片文件大小
            for image in "${REQUIRED_IMAGES[@]}"; do
                FILE_SIZE=$(stat -f%z "$INITIAL_DOCS_DEST/$image" 2>/dev/null || echo "0")
                if [ "$FILE_SIZE" -lt 1000 ]; then
                    echo "   ⚠️  警告: $image 文件大小异常小 ($FILE_SIZE 字节)"
                fi
            done
        else
...
echo "   ✓ 初始文档已包含（使用说明、Markdown完整示例、运动、技术）"
echo "   ✓ 应用图标已集成（Info.plist 包含 CFBundleIconFile）"
echo ""
echo "🚀 可以分发了！"
echo ""
