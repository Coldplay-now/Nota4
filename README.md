...
#### 🐛 Bug 修复
- **图片预览渲染修复**: 修复了笔记中插入的图片在预览模式下无法显示的问题
  - 使用 `loadFileURL` 方案替代 `loadHTMLString`，正确授予 WKWebView Sandbox 权限
  - 临时 HTML 文件放在笔记目录内，确保图片路径正确解析
  - 提供降级方案：如果临时文件创建失败，自动降级使用 HTML 字符串
  - 完善资源清理逻辑，避免临时文件泄漏

#### 🔧 技术改进
- 遵循 TCA 状态管理机制，在 `PreviewState` 中跟踪临时 HTML 文件路径
- 优化 `WebViewWrapper`，支持 `loadFileURL` 和 `loadHTMLString` 两种方案
- 改进错误处理和资源清理逻辑

### v1.3.0 (2025-11-21)

#### ✨ 新功能
- 导出 `.nota` 格式时，图片自动导出到子文件夹
- 帮助文档 HTML 预览功能
- 优化默认笔记内容（使用说明、Markdown 完整示例等）
